...
```{r}
na.patterns <- Hmisc::naclus(titanic3)
...
The options in the mice function indicate the number of $m$ replicated datasets (5 is the default) and the imputation method to use. "pmm" refers to predictive mean matching. Predictive mean matching combines the regression method and the hot deck imputation method by taking a donor value from the observation with the predicted value closest to the predicted value of the observation with missing data. This is beneficial because the imputed values will still be in a plausible range.
...
```{r}
with(titanic.imp, glm(survived ~ pclass + age + sex + parch, 
                      family="binomial"))
```
